...
function Process(action, start, tick, finish)
    ...
    if not IsEntityDead(ped) or Action.useWhileDead then
        if not isDoingAction then
            ...
            CreateThread(function ()
                ...
                while isDoingAction do
                    ...
                    if IsControlJustPressed(0, 200) and Action.canCancel then
                        TriggerEvent("pos:client:cancel")
                    end
            ...
            Core.Notification("You are already doing something!", "error")
        end
    else
        Core.Notification("Cant do that action!", "error")
    end
end
...
RegisterNetEvent('pos:client:progress', function(action, finish)
	Process(action, nil, nil, finish)
end)

RegisterNetEvent('pos:client:cancel', function()
	Cancel()
end)
